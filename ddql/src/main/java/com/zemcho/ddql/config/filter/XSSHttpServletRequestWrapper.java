package com.zemcho.ddql.config.filter;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.zemcho.ddql.util.XssUtil;
...
public class XSSHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    private String requestBodyToString(HttpServletRequest request) throws IOException {
        ...
        if (body.trim().startsWith("[")) {
            // 数组格式，直接返回（已在 requestBodyToMap 中处理过滤）
            return body;
        }
        
        // 对象格式，按原有逻辑处理
        Map<String, Object> map = JSON.parseObject(buffer, Map.class);
        filterMap(map);
