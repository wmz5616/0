...
class App extends React.Component {
  ...
  getData = () => {
    ...
    if (!this.props.add) {
      this.formRef.current.setFieldsValue({
        ...
        shopIds: edit.shops && edit.shops != 0 ? edit.shops.map((sx) => sx.id) : [],
      });
    }
    this.props.dispatch({
  ...
  render() {
    ...
    const { list = [], lists = [], placeLists = [] } = this.state;
    return (
      <>
        <Modal
        ...
        >
          <Form ref={this.formRef} labelCol={{ span: 6 }} wrapperCol={{ span: 16 }}>
            ...
            >
              <Form.Item name="account" noStyle rules={[{ required: true, message: '请输入' }]}>
            ...
            <Form.Item label={<span>密码</span>}>
              <Form.Item
                name="password"
                noStyle
                rules={[{ required: this.state.isAdd ? true : false, message: '请输入密码' }]}
              >
                <Input placeholder="请输入" />
              </Form.Item>
              <div style={{ color: '#ccc' }}>6~20位，仅支持数字、字母和下划线</div>
            </Form.Item>
