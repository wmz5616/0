...
class NoticeNotice extends React.Component {
    ...
    this.setState(
      ...
      () => {
        ...
        this.setState({
          ...
          licenceUrl: (xx.licenceUrlLocal && xx.licenceUrlLocal !== 'null') ? [
                {
                  uid: '-1',
                  name: 'image.png',
                  status: 'done',
                  url: xx.licenceUrlLocal.startsWith('http') ? xx.licenceUrlLocal : urlName + xx.licenceUrlLocal,
                  thumbUrl: xx.licenceUrlLocal.startsWith('http') ? xx.licenceUrlLocal : urlName + xx.licenceUrlLocal,
                },
              ]
            : [],

          licenceUrls: (xx.licenceUrlLocal && xx.licenceUrlLocal !== 'null') ? [
                {
                  uid: '-1',
                  name: 'image.png',
                  status: 'done',
          ...
          IdPhoto: (xx.legalLicenceFrontUrlLocal && xx.legalLicenceFrontUrlLocal !== 'null') ? [
                {
                  uid: '-1',
                  name: 'image.png',
                  status: 'done',
                  url: xx.legalLicenceFrontUrlLocal.startsWith('http') ? xx.legalLicenceFrontUrlLocal : urlName + xx.legalLicenceFrontUrlLocal,
                  thumbUrl: xx.legalLicenceFrontUrlLocal.startsWith('http') ? xx.legalLicenceFrontUrlLocal : urlName + xx.legalLicenceFrontUrlLocal,
                },
              ]
            : [], //身份证正面

          IdPhotoB: (xx.legalLicenceBackUrlLocal && xx.legalLicenceBackUrlLocal !== 'null') ? [
                {
                  uid: '-1',
                  name: 'image.png',
                  status: 'done',
                  url: xx.legalLicenceBackUrlLocal.startsWith('http') ? xx.legalLicenceBackUrlLocal : urlName + xx.legalLicenceBackUrlLocal,
                  thumbUrl: xx.legalLicenceBackUrlLocal.startsWith('http') ? xx.legalLicenceBackUrlLocal : urlName + xx.legalLicenceBackUrlLocal,
                },
              ]
            : [], //身份证反面
          ...
          openAccountLicenceUrl: (xx.openAccountLicenceUrlLocal && xx.openAccountLicenceUrlLocal !== 'null') ? [
                {
                  uid: '-1',
                  name: 'image.png',
                  status: 'done',
                  url: xx.openAccountLicenceUrlLocal.startsWith('http') ? xx.openAccountLicenceUrlLocal : urlName + xx.openAccountLicenceUrlLocal,
                  thumbUrl: xx.openAccountLicenceUrlLocal.startsWith('http') ? xx.openAccountLicenceUrlLocal : urlName + xx.openAccountLicenceUrlLocal,
                },
              ]
            : [], //*开户许可证照片
        });
  ...
  //         },
  //         url: `/ddql/merchant/image/upload`,
  //         method: 'POST',
  //         myData: (res) => {
          ...
          <div style={{ backgroundColor: '#fff', paddingTop: 50, paddingBottom: 50 }}>
            <Form ref={this.formRef} {...layout}>
              ...
              <Form.Item label="进件类型" name="operationType" initialValue={operationTypes}>
                <Radio.Group disabled onChange={this.onChange}>
              ...
              <Form.Item label="联系电话" name="contactPhone" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入商户联系电话" />
              </Form.Item>
              <Form.Item label="邮箱地址" name="email" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入商户邮箱地址" />
              </Form.Item>
              <Form.Item label="门店名称" name="storeName" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入门店名称" />
              </Form.Item>
              ...
              <Form.Item label="详细地址" name="storeAddr" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入门店详细地址" />
              </Form.Item>
              ...
              {operationTypes == 1 ? (
                <>
                  ...
                  <Form.Item label="主体名称" name="businessName" rules={[{ required: true }]}>
                    <Input disabled placeholder="请根据主体营业执照或登记证书准确输入主体名称" />
                  </Form.Item>
                  ...
                  <Form.Item label="执照/证书编号" name="businessNo" rules={[{ required: true }]}>
                    <Input disabled placeholder="请输入主体营业执照或登记证书编号" />
                  </Form.Item>

                  <Form.Item label="法人姓名" name="legalPerson" rules={[{ required: true }]}>
                    <Input disabled placeholder="请根据证件信息准确输入主体法定代表人姓名" />
                  </Form.Item>

                  <Form.Item label="所属地区" name="businessProvince" rules={[{ required: true }]}>
                    <Cascader disabled options={this.state.bbb} placeholder="请选择" />
                  </Form.Item>

                  <Form.Item label="详细地址" name="businessAddr" rules={[{ required: true }]}>
                    <Input disabled placeholder="请根据主体营业执照或登记证书准确输入详细地址" />
                  </Form.Item>
                  ...
                  <Form.Item
                    ...
                    rules={[{ required: true }]}
                  >
                    <Radio.Group disabled onChange={this.onChanges} initialValue={this.state.effectiveType}>
              ...
              <Form.Item label="证件持有人姓名" name="cardName" rules={[{ required: true }]}>
                <Input disabled placeholder="请根据证件信息准确输入证件持有人姓名" />
              </Form.Item>
              <Form.Item label="证件号码" name="cardNo" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入身份证号码" />
              </Form.Item>
              <Form.Item label="手机号码" name="cardMobile" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入证件持有人手机号码" />
              </Form.Item>
              ...
              <Form.Item label="证件有效期" name="documentType" rules={[{ required: true }]}>
                <Radio.Group disabled onChange={this.onChangeg}>
                  <Radio value={0}>有效期</Radio>
                  <Radio value={1}>长期</Radio>
                </Radio.Group>
              </Form.Item>
              ...
              >
                <Form.Item name="accCardNo" noStyle>
                  <Input disabled placeholder="请输入银行账号" />
                </Form.Item>
              ...
              >
                <Form.Item name="accName" noStyle>
                  <Input disabled placeholder="请输入银行账户名" />
                </Form.Item>
              ...
              <Form.Item label="开户预留手机" name="accMobile" rules={[{ required: true }]}>
                <Input disabled placeholder="请输入银行账户开户预留手机" />
              </Form.Item>
              ...
              >
                <Select disabled allowClear showSearch optionFilterProp="label" placeholder="请选择开户行">
                  {aaas.map((res) => {
