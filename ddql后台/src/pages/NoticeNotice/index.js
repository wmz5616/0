...
class NoticeNotice extends React.Component {
    ...
    this.setState(
      {
        ...
        startTime: res.publishTime && res.publishTime[0].format('YYYY-MM-DD 00:00:00'),
        endTime: res.publishTime && res.publishTime[1].format('YYYY-MM-DD 23:59:59'),
        pageNum: 1,
      },
      () => {
        this.getData();
  ...
  render() {
    const { list = [], selectedRowKeys, NewRoles, edit, xxx } = this.state;
    const rowSelection = {
      selectedRowKeys,
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              <span>
                {text == 1 ? <span className="luSe">是</span> : <span className="red">否</span>}
              </span>
            </div>
          );
        },
          ...
          return (
            <div>
              <span>
                {text ||
                (record.publishTime &&
                  new Date(record.publishTime).getTime() < new Date().getTime()) ? (
        ...
        render: (text, record) => {
          return (
            <div>
              ...
              {record.isTop == 1 ? (
                <a className="mL15" onClick={() => this.siteFrontReserves(0, [record.id])}>
                  取消置顶
                </a>
              ) : (
                <a className="mL15" onClick={() => this.siteFrontReserves(1, [record.id])}>
                  置顶
                </a>
              )}
        ...
        <Spin spinning={this.state.spinning}>
          <div style={{ backgroundColor: '#fff', padding: '20px 20px 0 20px', marginBottom: 15 }}>
            <Form ref={this.formRef} onFinish={this.onFinish}>
              <Row gutter={16}>
                ...
                {/* {xxx && (
                  <Col className="gutter-row" span={6}>
                    <Form.Item label="发布时间" name="usernames">
          ...
          <div style={{ backgroundColor: '#fff', padding: 20 }}>
            <Row>
              <Col span={6}>
                {' '}
                <h1 style={{ fontWeight: '600', fontSize: '18px' }}>通知公告</h1>
              </Col>
              <Col span={18}>
