...
import { getSubLedgerDetailList, getSubLedgerSummary, exportSubLedgerDetail } from '@/services/transaction';
...
const BillingDetails = () => {
  ...
  const [searchParams, setSearchParams] = useState({
    startTime: location.state?.startTime || dayjs().startOf('month').format('YYYY-MM-DD 00:00:00'),
    endTime: location.state?.endTime || dayjs().endOf('day').format('YYYY-MM-DD 23:59:59'),
    pageNum: 1,
    pageSize: 10,
  });
  ...
  const columns = [
    ...
    { 
      ...
      key: 'divideAmount', 
      width: 130,
      render: (val) => <span style={{ color: val < 0 ? '#ff4d4f' : 'inherit' }}>{Number(val).toFixed(2)}</span> 
    },
    { 
      title: '通莞手续费 (元)', 
        ...
        <Form form={form} onFinish={onFinish}>
          <Row gutter={16}>
            <Col className="gutter-row" span={5}>
              <Form.Item label="分账时间" name="time" initialValue={[dayjs().startOf('month'), dayjs()]}>
            ...
            >
              <Form.Item>
                ...
                <Button className="mL15" onClick={handleReset} style={{ marginLeft: 8 }}>重置</Button>
              </Form.Item>
            </Col>
          </Row>
        </Form>
            ...
            <Alert
              message={
                <Space split={<span>|</span>} size="middle">
                  <span>
                    查询统计：共 <Text strong style={{ color: '#ff8800' }}>{summary.totalCount || 0}</Text> 笔
                  </span>
                  <span>
                    分账金额 <Text strong style={{ color: '#ff8800' }}>¥{Number(summary.totalDivideAmount || 0).toFixed(2)}</Text>
                  </span>
                  <span>
                    通莞手续费 <Text strong style={{ color: '#ff8800' }}>¥{Number(summary.totalTongGuanFee || 0).toFixed(2)}</Text>
                  </span>
                  <span>
                    平台收费 <Text strong style={{ color: '#ff8800' }}>¥{Number(summary.totalPlatformFee || 0).toFixed(2)}</Text>
                  </span>
                </Space>
              }
              type="info"
        ...
        <Table
          ...
          pagination={{
            ...
            onChange: (page, pageSize) => {
              const newParams = { ...searchParams, pageNum: page, pageSize: pageSize || searchParams.pageSize };
              setSearchParams(newParams);
              loadData(newParams);
            },
          }}
