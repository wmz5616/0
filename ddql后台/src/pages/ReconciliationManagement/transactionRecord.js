...
const TransactionRecord = () => {
  ...
  const [searchParams, setSearchParams] = useState({
    startTime: initialState.startTime || dayjs().startOf('month').format('YYYY-MM-DD 00:00:00'),
    endTime: initialState.endTime || dayjs().endOf('day').format('YYYY-MM-DD 23:59:59'),
    pageNum: 1,
    pageSize: 10,
  });
  ...
  const loadData = async (params) => {
    ...
    try {
      ...
      const finalData = {
        list: listRes?.code === 10000 ? listRes.data?.list || [] : [],
        total: listRes?.code === 10000 ? listRes.data?.total || 0 : 0,
        summary: summaryRes?.code === 10000 ? summaryRes.data || {} : {},
      };
  ...
  useEffect(() => {
    const targetParams = (initialState?.startTime && initialState?.endTime) ? {
      ...searchParams,
    ...
    if (initialState?.startTime) {
      form.setFieldsValue({ time: [dayjs(initialState.startTime), dayjs(initialState.endTime)] });
            ...
            <Alert
              message={
                <Space split={<span>|</span>} size="middle">
                  <span>
                    查询统计：共{' '}
                    <Text strong style={{ color: '#ff8800' }}>{summary.totalCount || 0}</Text> 笔
                  </span>
                  <span>
                    收入 <Text strong style={{ color: '#ff8800' }}>{summary.incomeCount || 0}</Text> 笔， 金额{' '}
                    <Text strong style={{ color: '#ff8800' }}>
                      ￥{(summary.incomeAmount || 0).toFixed(2)}
                    </Text>
                  </span>
                  <span>
                    支出 <Text strong style={{ color: '#ff8800' }}>{summary.expenseCount || 0}</Text> 笔， 金额{' '}
                    <Text strong style={{ color: '#ff8800' }}>
