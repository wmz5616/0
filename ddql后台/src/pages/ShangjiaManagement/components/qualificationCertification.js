...
class QualificationCertification extends React.Component {
  ...
  onFinish = () => {
    this.formRef.current.validateFields().then((values) => {
      this.props.dispatch({
        ...
        payload: {
          businessLicense: this.state.thumbnailFileList.map((cz) => cz.response.data.url)[0],
          otherFile: this.state.carouselFileList.map((cz) => cz.response.data.url).join(','),
          ...values,
          shopId: this.state.id,
          id: this.state.searchId || undefined,
        },
        url: this.state.searchId
          ? '/ddql/business/shop/qualification/update'
          : `/ddql/business/shop/qualification/add`,
        method: 'POST',
        myData: (res) => {
  ...
  getData = () => {
    ...
    this.props.dispatch({
      ...
      url: `/ddql/business/shop/qualification/get`,
      method: 'POST',
      myData: (res) => {
        this.setState({
          spinning: false,
          ...
          this.setState(
            {
              ...
              carouselFileList: data.otherFile.split(',').map((url, index) => ({
                uid: String(index + 1),
                name: 'image.png',
                status: 'done',
                url,
  ...
  review = () => {
    this.formRefs.current.validateFields().then((values) => {
      this.props.dispatch({
        ...
        url: '/ddql/business/shop/qualification/audit',
        method: 'POST',
        myData: (res) => {
          if (res && res.code === 10000) {
            this.setState({
  ...
  render() {
    const { certResult, thumbnailFileList, carouselFileList, type, disabled } = this.state;
    const props = {
      grid: false,
      width: 690,
      height: 312,
    ...
    return (
      <Spin spinning={this.state.spinning}>
        <Row align="middle" justify="space-between">
          <Col span={4} style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            <h2>资质认证</h2>
            <Tooltip
              placement="top"
              title={certResult == 2 ? <div>审核驳回的原因：{this.state.rejectReason}</div> : null}
            >
              <div
                style={{
                  color:
        ...
        >
          ...
          <Form.Item
            label={
              <span>
                <span style={{ color: 'red' }}>*</span>其他附件
              </span>
            }
          >
            <Form.Item noStyle>
          ...
          >
            <Form.Item noStyle name="phone" rules={[{ required: true, message: '请输入！' }]}>
              <Input placeholder="请输入" />
            </Form.Item>
            <span style={{ color: '#ccc' }}>认证过程会致电该电话，认证期间请保诗电话畅通。</span>
          </Form.Item>
          <Form.Item label="邮箱" name="email" rules={[{ required: false, message: '请输入！' }]}>
            <Input placeholder="请输入" />
          </Form.Item>
          <Form.Item>
            {type != 'info' && (
                  ...
                  <Popconfirm
                    ...
                    title={
                      <>
                        <div>确认提交</div>
                        <div>
                          <text style={{ color: 'red' }}>提交之后自动进行资质审核</text>
                          <text style={{ color: '#999999' }}>，确定提交吗？</text>
                        </div>
                      </>
                    }
...
export default connect()(QualificationCertification);
