...
import NewVenues from './components/AddShopMsgModal';
// import { setToken } from '@/utils/authority';
const { Option } = Select;
const { RangePicker } = DatePicker;
const { TextArea } = Input;

class ShotMsgList extends React.Component {
        ...
        <Spin spinning={this.state.spinning}>
                ...
                <Col className="gutter-row" span={5} style={{ marginRight: 30 }}>
                  <Form.Item label="所属商圈" name="searchId">
                    <Select allowClear placeholder="请选择">
                      {this.state.circleList.map((sxsa) => (
          ...
          <div style={{ backgroundColor: '#fff', padding: 20 }}>
            <Row>
              <Col span={6}>
                {' '}
                <h1 style={{ fontWeight: '600', fontSize: '18px' }}>店铺信息管理</h1>
              </Col>
              <Col span={18}>
...
export default connect()(ShotMsgList);
