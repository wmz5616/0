...
class DataConnection extends React.Component {
    ...
    this.setState(
      ...
      () => {
        ...
        this.props.dispatch({
          ...
          payload: {},
          url: `/ddql/homePage/banner/allList`,
          method: 'POST',
          myData: (res) => {
            this.setState({
              spinning: false,
        ...
        this.setState(
          ...
          () => {
            ...
            this.props.dispatch({
              ...
              url: `/ddql/homePage/banner/sort/set`,
              method: 'POST',
              myData: (res) => {
                if (res && res.code === 10000) {
  ...
  handleOk = () => {
    ...
    this.formRefs.current.validateFields().then((values) => {
      ...
      dispatch({
        ...
        method: 'POST',
        url: this.state.add ? `/ddql/homePage/banner/insert` : `/ddql/homePage/banner/update`,
        myData: (res) => {
          if (res.code === 10000) {
            message.success(res.msg);
            this.setState({
  ...
  operationStatus = (e, id) => {
    ...
    this.props.dispatch({
      ...
      url: `/ddql/homePage/banner/status/set`,
      method: 'POST',
      myData: (res) => {
        if (res && res.code === 10000) {
          message.success(res.msg);
  ...
  deletes = (ids) => {
    if (ids.length == 0) {
      ...
      this.props.dispatch({
        ...
        url: `/ddql/homePage/banner/delete`,
        method: 'POST',
        myData: (res) => {
          if (res && res.code === 10000) {
            message.success(res.msg);
        ...
        render: (text, record) => {
          return (
            <div>
              ...
              {record.status == 1 ? (
                <a className="mL15" onClick={() => this.operationStatus(0, [record.id])}>
                  禁用
                </a>
              ) : (
                <a className="mL15" onClick={() => this.operationStatus(1, [record.id])}>
                  启用
                </a>
              )}
              <Popconfirm
      ...
      <Spin spinning={this.state.spinning}>
        <div style={{ backgroundColor: '#f0f2f5', paddingBottom: 15 }}>
          <div style={{ padding: 24, backgroundColor: '#fff', paddingBottom: 0 }}>
            <Row>
              <Col span={6}>
                <h3 style={{ fontWeight: '600', fontSize: '18px', margin: 0 }}>首页轮播图</h3>
              </Col>
              <Col span={18} style={{ textAlign: 'right' }}>
                ...
                <Button
                  ...
                  onClick={() => this.operationStatus(1, this.state.selectedRowKeys)}
                >
                  启用
                </Button>
                <Button
                  className="mL15 bxHuang"
                  onClick={() => this.operationStatus(0, this.state.selectedRowKeys)}
                >
                  禁用
                </Button>
