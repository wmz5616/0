...
class Login extends React.Component {
  ...
  deletes = (e) => {
    this.props.dispatch({
      ...
      url: `/api/admin/user/certification/cancel`,
      method: 'POST',
      myData: (res) => {
  ...
  export = () => {
    ...
    xhr.open('POST', `/ddql/user/export`, true);
  ...
  render() {
    const { gymTypelist = [], selectedRowKeys, NewRoles, total, pageNum } = this.state;
    const rowSelection = {
      selectedRowKeys,
    ...
    const columns = [
      ...
      },
      {
        title: '实名认证',
        dataIndex: 'name',
        render: (res, record) => <div>{record.has_certification == 1 ? res : '未实名'}</div>,
      },
...

      {
        ...
        render: (text, record) => {
          return (
            <div>
              <a onClick={() => this.showModal(true, [record.id], record)}>编辑状态</a>
              <Link className="mL15" to={`UserDetails?id=${record.id}`}>
                详情
              </Link>
            </div>
          );
    ...
    return (
      ...
      >
        <Spin spinning={this.state.spinning}>
          ...
          <div
            style={{ backgroundColor: '#fff', padding: 20, minHeight: window.innerHeight - 310 }}
          >
            <Row>
              <Col span={6}>
                {' '}
                <h1 style={{ fontWeight: '600', fontSize: '18px' }}>用户管理</h1>
              </Col>
              <Col span={18}>
