...
Page({
  ...
  getruzhuAuditList: function () {
    ...
    ruzhuAuditList({
      searchIntStatus: type
    }, resd => {
      const list=resd.data.list || []
      list.map(a=>{
        a.submitTime=moment(a.submitTime).format('YYYY-MM-DD HH:mm:ss')
      })
      this.setData({
        listData:list
      })
  ...
  goDetail: function (e) {
    wx.navigateTo({
      url: `/pagesA/applySettlement/applySettlement?shopId=${e.currentTarget.dataset.id}`,
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
