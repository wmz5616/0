...
Page({
  ...
  submit: function () {
    ...
    const params = {
      ...
      galleryImages: this.data.bannerList.map(c => c.url).join(),
      name: this.data.name,
  ...
  afterRead(event) {
    ...
    const processFile = (index) => {
      ...
      const f = files[index];
      uploadImages(f.tempFilePath, resd => {
  ...
  getshopaudit: function () {
    ...
    // 商家详情
      ...
      getshopaudit({
        ...
        const industriesData = resd.data.industryIds.map(q => q)
        const circlesData = resd.data.circleIds.map(q => q)
        ...
        resd.data.settlementApplication.submitTime=moment(resd.data.settlementApplication.submitTime).format('YYYY-MM-DD HH:mm:ss')
        this.setData({
        ...
        this.setData({
          ...
          bannerList: formData.galleryImages ? JSON.parse(formData.galleryImages).map(xzc => {
            return {
              url: xzc
            }
          }) : [],
