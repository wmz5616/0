...
public class AppVersionController {
    ...
    public Result delete(@Valid @RequestBody SearchParam param, BindingResult result){
        if (result.hasErrors()) {
            return Result.error(result.getFieldError().getDefaultMessage());
        }
        return appVersionService.delete(param.getSearchId());
    }
    ...
    public Result selectLog(@Valid @RequestBody SearchParam param, BindingResult result){
        if (result.hasErrors()) {
            return Result.error(result.getFieldError().getDefaultMessage());
        }
        return appVersionService.selectLog(param);
    }
    ...
    @Log(description = "发布版本", module = "App版本管理")
    public Result publish(@Valid @RequestBody SearchParam param, BindingResult result){
        if (result.hasErrors()) {
            return Result.error(result.getFieldError().getDefaultMessage());
        }
        return appVersionService.publish(param);
    }
