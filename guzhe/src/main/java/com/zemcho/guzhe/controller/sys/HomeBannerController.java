...
public class HomeBannerController {
    ...
    /*/**
        * @description: 获取首页快捷入口数据或者首页banner图
        * @author Ryan
    */
    @RequestMapping("/allList")
    public Result getBannerAllList(
            @Validated @RequestBody SearchParam param,
            BindingResult result)
    {
        if (result.hasErrors()) {
            return new Result(10002, result.getFieldError().getDefaultMessage());
        }
        return homeBannerService.selectAll(param);
    }

    @RequestMapping("/showList")
    public Result getBannerShowList() {
        return homeBannerService.getBannerShowList();
    }
}
