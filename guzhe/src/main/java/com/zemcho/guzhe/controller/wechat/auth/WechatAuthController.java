...
public class WechatAuthController {
    ...
    public Result updatePhone(@Validated @RequestBody WechatBindPhoneParam param, BindingResult result, @RequestHeader("token") String token) {
        if (result.hasErrors()) {
            return Result.error(result.getFieldError().getDefaultMessage());
        }
        // 从 Token 中解析当前用户 ID
        Integer userId = com.zemcho.guzhe.config.jwt.JWTUtil.getIntClaim(token, com.zemcho.guzhe.util.Constant.MINI_USER_ID);
        if (userId == null) {
            return new Result(10006, "token无效");
        }
        return service.updateUserPhone(param, token);
    }
     ...
     * @return
     */
    @PostMapping("/update/phone_by_wechat")
    public Result updatePhoneByWechat(@Validated @RequestBody WechatAuthParam param, BindingResult result, @RequestHeader("token") String token) {
        if (result.hasErrors()) {
            return Result.error(result.getFieldError().getDefaultMessage());
        }
        // 从 Token 中解析当前用户 ID
        ...
        return service.updatePhoneByWechat(param, token);
    }
}
