...
public class AuditShopController {
    ...
    public Result submitQualification(@Validated @RequestBody WechatQualificationCertParam qualificationCertParam, BindingResult result,
                                      @RequestHeader("token") String token) {
        if (result.hasErrors()) {
            return new Result(10002, result.getFieldError().getDefaultMessage());
        }
    ...
    public Result getQualificationCertDetail(@Validated @RequestBody SearchParam param, BindingResult result,
                                             @RequestHeader("token") String token) {
        if (result.hasErrors()) {
            return new Result(10002, result.getFieldError().getDefaultMessage());
        }
        return auditShopService.getQualificationCertDetail(param,token);
    }
    /**
     * 小程序查询申请入驻记录
     *
    ...
    public Result getMerchantNotice() {
        return auditShopService.getMerchantNotice();
    }
    ...
    public Result getCount(@RequestHeader("token") String token) {
        return auditShopService.getCount(token);
    }
