...
public class UserProductOrderController {
    ...
    public Result productOrderProductInfo(@Validated @RequestBody ProductOrderAddParam param, BindingResult result) {
        if (result.hasErrors()) {
            return new Result(10002, result.getFieldError().getDefaultMessage());
        }
        return service.productOrderProductInfo(param);
    }
    ...
    public Result productOrderCancel(@Validated @RequestBody SearchParam param,
                                     BindingResult result, @RequestHeader("token") String token) {
        if (result.hasErrors()) {
            return new Result(10002, result.getFieldError().getDefaultMessage());
        }
        return service.productOrderCancel(param, token);
    }
