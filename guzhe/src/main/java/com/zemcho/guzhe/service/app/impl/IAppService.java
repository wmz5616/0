...
import com.zemcho.guzhe.controller.app.param.AppEquipmentScreenshotSaveParam;
...
public class IAppService implements AppService {
    ...
    public Result updateVersion(AppVersionUpdateParam param, String accessToken) {
        ...
        //app版本发布日志
        LocalDateTime now = LocalDateTime.now();
        AppVersionPushLog versionLog = new AppVersionPushLog();
        versionLog.setSerialNumber(equipmentInfo.getSerialNumber());
        versionLog.setDeviceId(equipmentId);
        versionLog.setSupId(equipmentInfo.getSupermarketId());
        versionLog.setStatus(param.getStatus());//发布状态
        versionLog.setCreateTime(now);
        appVersionPushLogMapper.insert(versionLog);
...
//        }
        SearchParam screenRentalOrderParam = new SearchParam();
        screenRentalOrderParam.setSearchField1(equipmentId);
        screenRentalOrderParam.setSearchIntStatus(2);
        List<ScreenRentalOrder> screenRentalOrderList = screenRentalOrderMapper.selectLists(screenRentalOrderParam);
        if (screenRentalOrderList == null || screenRentalOrderList.isEmpty()) {
            return Result.success("获取成功", new ArrayList<>());
        }
        List<Integer> shopIds = new ArrayList<>();
//        Map<Integer, ScreenRentalOrder> screenRentalOrderMap = new HashMap<>();
        for (ScreenRentalOrder screenRentalOrder : screenRentalOrderList) {
            shopIds.add(screenRentalOrder.getShopId());
//            screenRentalOrderMap.put(screenRentalOrder.getShopId(), screenRentalOrder);
        }

        List<AppShopListVo> shopList = shopMapper.selectAppShopListsByShopIds(shopIds);
        List<AppShopListVo> list = new ArrayList<>();
        if (shopList != null && !shopList.isEmpty()) {
            ...
            Map<Integer, AppShopListVo> shopMap = shopList.stream().collect(Collectors.toMap(AppShopListVo::getId, v -> v));

            for (ScreenRentalOrder screenRentalOrder : screenRentalOrderList) {
                Integer shopId = screenRentalOrder.getShopId();
                AppShopListVo originalShopInfo = shopMap.get(shopId);
                if (originalShopInfo == null) {
                    continue;
                }

                //这里进行深拷贝处理，避免存在多个商家id的数据导致数据都是用的同一个shopInfo对象
                AppShopListVo shopInfo = new AppShopListVo();
                BeanUtils.copyProperties(originalShopInfo, shopInfo);
...
//                ScreenRentalOrder screenRentalOrder = screenRentalOrderMap.get(shopInfo.getId());
//                if (screenRentalOrder != null) {
//                    shopInfo.setDisplayType(screenRentalOrder.getDisplayType());
//                }
                shopInfo.setDisplayType(screenRentalOrder.getDisplayType());
     ...
     */
    @Override
    public Result getMerchantApplySettlementQRCode(String accessToken) {
        AppJwtData appJwtData = JWTUtil.getAppAuthJwtData(accessToken);
        if (appJwtData == null) {
            return new Result(10006, "token无效");
        }
        ...
        String scene = equipmentId.toString();
        String fileName = scene + "_" + Md5Util.MD5(appJwtData.getSerialNumber()) + ".png";
        String filePath = localFilePath + uploadFilePath + "/merchant_apply_settlement_qr/";
        WechatUtil.generateAndSaveQrCode(scene, wechatConfig.getMerchantApplySettlementQRPage(), 430,
                wechatConfig.getCodeQREnv(), filePath, fileName);

        Map<String, Object> result = new HashMap<>();
        result.put("filePath", uploadFilePath + "/merchant_apply_settlement_qr/" + fileName);
    ...
    @Override
    public Result getEquipmentScreenshotTask(String accessToken) {
        AppJwtData appJwtData = JWTUtil.getAppAuthJwtData(accessToken);
        if (appJwtData == null) {
            return new Result(10006, "token无效");
        }
        ...
        Integer equipmentId = appJwtData.getId();
        String cacheKey = Constant.EQUIPMENT_SCREENSHOT_TASK_PREFIX + equipmentId;
        Object value = redisUtil.get(cacheKey);

        Map<String, Object> result = new HashMap<>();
        result.put("needScreenshot", false);
        if (!(value instanceof Map<?, ?> taskData)) {
            return Result.success("获取成功", result);
        }

        Object needScreenshot = taskData.get("needScreenshot");
        if (!(needScreenshot instanceof Boolean) || !((Boolean) needScreenshot)) {
            return Result.success("获取成功", result);
        }

        result.put("needScreenshot", true);
        result.put("screenshotId", taskData.get("screenshotId"));
        redisUtil.del(cacheKey);
        return Result.success("获取成功", result);
    }

    @Override
    public Result saveEquipmentScreenshot(AppEquipmentScreenshotSaveParam param, String accessToken) {
        AppJwtData appJwtData = JWTUtil.getAppAuthJwtData(accessToken);
        if (appJwtData == null) {
            return new Result(10006, "token无效");
        }
        ...
        EquipmentScreenshot screenshot = equipmentScreenshotMapper.selectById(param.getScreenshotId());
        if (screenshot == null) {
            return Result.error("设备截图记录不存在");
        }
        if (!equipmentId.equals(screenshot.getEquipmentId())) {
            return Result.error("设备截图记录不匹配");
        }

        String failReason = param.getFailReason() == null ? "" : param.getFailReason().trim();
        String base64Image = param.getBase64Image() == null ? "" : param.getBase64Image().trim();

        EquipmentScreenshot updateData = new EquipmentScreenshot();
        updateData.setId(screenshot.getId());
        updateData.setScreenshotStatus(param.getScreenshotStatus());
        if (!failReason.isEmpty()) {
            updateData.setScreenshotStatus(2);
            updateData.setFailReason(failReason);
            updateData.setScreenshotUrl("");
            equipmentScreenshotMapper.update(updateData);
            return Result.success("保存成功");
        }

        if (base64Image.isEmpty()) {
            return Result.error("截图内容不能为空");
        }

        try {
            // 解码图片数据
            ImageData imageData = decodeBase64Image(base64Image);
            String fileDir = localFilePath + uploadFilePath + "/device_screenshot/" + equipmentId + "/";
            Path dirPath = Paths.get(fileDir);
            FileUploadUtil.createDirectoryWithPermissions(dirPath.toFile(), "rwxr-xr-x");

            String fileName = screenshot.getId() + "_" + System.currentTimeMillis() + "." + imageData.extension;
            Path filePath = dirPath.resolve(fileName);
            Files.write(filePath, imageData.content);
            FileUploadUtil.setPosixPermissions(filePath, "rw-r--r--");

            //updateData.setScreenshotStatus(1);
            updateData.setFailReason("");
            updateData.setScreenshotUrl(uploadFilePath + "/device_screenshot/" + equipmentId + "/" + fileName);
            equipmentScreenshotMapper.update(updateData);
            return Result.success("保存成功");
        } catch (Exception e) {
            updateData.setScreenshotStatus(2);
            updateData.setFailReason("截图保存失败：" + e.getMessage());
            updateData.setScreenshotUrl("");
            equipmentScreenshotMapper.update(updateData);
            return Result.error("截图保存失败");
        }
    }

    private ImageData decodeBase64Image(String base64Image) {
        String data = base64Image;
        String extension = "png";
        if (base64Image.startsWith("data:image/")) {
            int slashIndex = base64Image.indexOf('/');
            int semicolonIndex = base64Image.indexOf(';');
            if (slashIndex > -1 && semicolonIndex > slashIndex) {
                extension = base64Image.substring(slashIndex + 1, semicolonIndex).toLowerCase();
            ...
            if (commaIndex > -1) {
                data = base64Image.substring(commaIndex + 1);
            }
        }
        ImageData imageData = new ImageData();
        imageData.extension = extension;
        imageData.content = Base64.getDecoder().decode(data);
        return imageData;
    }
