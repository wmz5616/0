...
import com.zemcho.guzhe.mapper.bank.BankMapper;
...
//        redisTemplate.delete(param.getUuid());
        ...
        // 检查短信发送频率限制
        if (!CodeRedisUtil.canSendSms(phone)) {
            return Result.error("获取太频繁，请稍后再试");
        }
        ...
        if (sendResult.getCode() == 10000) {
            ...
            // 标记短信已发送，设置频率限制
            CodeRedisUtil.markSmsSent(phone);
     ...
     */
    @Override
    public Result getWechatSupermarketLists(WechatSupermarketListParam param) {
        String addressKeyword = buildWechatAddressKeyword(param);
        List<SupermarketInfo> supermarkets = supermarketMapper.selectEnableByAddressKeyword(addressKeyword);
        List<SupermarketCommonVo> list = supermarkets.stream().map(item -> {
            SupermarketCommonVo vo = new SupermarketCommonVo();
            vo.setId(item.getId());
    ...
    @Override
    public Result getWechatBankLists(WechatBankQueryParam param) {
        param.setIsFilterNullBank(true);
        List<WechatBankCommonVo> list = bankMapper.selectBankLists(param);
        return Result.success("获取成功", list);
    }

    @Override
    public Result getWechatBankProvinceCityLists(WechatBankQueryParam param) {
        List<WechatBankProvinceCityVo> list = bankMapper.selectProvinceCityLists(param);
        return Result.success("获取成功", list);
    }

    @Override
    public Result getWechatBankLinkLists(WechatBankQueryParam param) {
        List<WechatBankLinkVo> list = bankMapper.selectBankLinkLists(param);
        return Result.success("获取成功", list);
    }

    /**
     * 将前端传入的省市区按顺序拼接成地址关键词，用于模糊匹配商超地址。
     */
    private String buildWechatAddressKeyword(WechatSupermarketListParam param) {
        StringBuilder builder = new StringBuilder();
        if (param.getProvince() != null && !param.getProvince().trim().isEmpty()) {
            builder.append(param.getProvince().trim());
        }
        if (param.getCity() != null && !param.getCity().trim().isEmpty()) {
            builder.append(param.getCity().trim());
        }
        if (param.getDistrict() != null && !param.getDistrict().trim().isEmpty()) {
            builder.append(param.getDistrict().trim());
        }
