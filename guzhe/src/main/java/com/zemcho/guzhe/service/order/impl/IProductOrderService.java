...
public class IProductOrderService implements ProductOrderService {
    ...
    public Result orderRefund(OrderRefundParam param, String token, Boolean isWechat) {
        ...
        log.info("orderRefund 操作人手机号:{} 订单:{} 退库商品:{} 退库数量:{} 券码id:{} 券码:{}", phone, orderId,
                orderInfo.getProductId(), num, ticketIds, ticketList);
        ...
        try {
            ...
            //更新商品库存数和兑换数
            if (num != 0) {
                productMapper.updateStockOrSaleNum(orderInfo.getProductId(), num, -num);
            }
        ...
        wechatSubscribeMessageService.notifyDirectRefundToUser(orderInfo, refundAmount, param.getRefundReason());
    ...
    public Result orderRefundAudit(OrderAuditParam param, String token, Boolean isWechat) {
        ...
        if (status == 2) { // 通过
            ...
            statusStr = "审核通过，退款申请原因：" + applyInfo.getReason();
        } else { // 驳回
        ...
        if (status == 2) {
            ...
            log.info("orderRefundAudit 操作人手机号:{} 订单:{} 退库商品:{} 退库数量:{} 券码id:{} 券码:{}", phone, orderId,
                    orderInfo.getProductId(), num, ticketIds, ticketList);
        }
        ...
        wechatSubscribeMessageService.notifyRefundResultToUser(orderInfo, applyInfo);
...
//        }
        ...
        //检查核销该劵码的商家是否是这个商品的商家
        Integer productId = ticketInfo.getProductId();
        Product product = productMapper.selectById(productId);
        Integer shopId = product.getShopId();//获取到该商品所属商家

        if(!shopId.equals(param.getSearchId())){
            return Result.error("您无权限核销");
        }
