...
public class ISupermarketService implements SupermarketService {
    ...
    public Result select(SearchParam param) {
        ...
        List<SupermarketInfoVo> voList = supermarketMapper.select(param);

        PageInfo<SupermarketInfoVo> voPageInfo = new PageInfo<>(voList);

        List<SupermarketInfo> infoList = new ArrayList<>();
        for (SupermarketInfoVo vo : voList) {
        ...
        PageInfo<SupermarketInfo> pageInfo = new PageInfo<>();
        BeanUtils.copyProperties(voPageInfo, pageInfo);
