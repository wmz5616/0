...
public class IUserProductOrderService implements UserProductOrderService {
    ...
    public Result addProductOrder(ProductOrderAddParam param, String token) {
        ...
        if (productInfo.getStock() < num) { //目前时间不够暂时不使用redis预热
            // 解锁
            redisLockUtil.unlock(lockKey);
            return Result.error("商品库存不足,目前的库存数量为:"+productInfo.getStock());
        }
        ...
        // 提醒库存不足
        Integer afterStock = productInfo.getStock() == null ? null : productInfo.getStock() - num;
        wechatSubscribeMessageService.notifyLowStockToShopManagers(productInfo, afterStock);

        // 提醒订单支付成功
        if (amount == 0 && createdOrder != null) {
            wechatSubscribeMessageService.notifyOrderPaidToShopManagers(createdOrder);
        }
     ...
     */
    @Override
    public Result productOrderProductInfo(ProductOrderAddParam param) {
        Integer productId = param.getProductId();
        Product productInfo = productMapper.selectById(productId);
        if (productInfo == null) {
            return Result.error("商品不存在");
        ...
        result.put("customerPhone", shopInfo.getCustomerPhone());
        result.put("customerCodeImg", shopInfo.getCustomerCodeImg());
    ...
    @Override
    @Transactional(propagation = Propagation.NEVER)
    public Result productOrderCancel(SearchParam param, String token) {
        Integer userId = JWTUtil.getIntClaim(token, Constant.MINI_USER_ID);
        if (userId == null) {
            return new Result(10006, "token无效");
        }
        ...
        if (!orderInfo.getStatus().equals(0)) {
            return Result.error("只有待支付的订单才可取消");
        }
        ...
        try {
            ...
            orderLog.setHandle("取消订单");
            orderLog.setDetails(orderInfo.getNickName() + "  " + orderInfo.getPhone() + "取消订单");
            orderLog.setCreateTime(now);
            productOrderLogMapper.insert(orderLog);
            ...
            log.error("productOrderCancel Error ", e);

            return Result.error("操作失败");
        }

        // 添加到订单取消退库延时处理监控--退库进行延时处理，避免在取消前操作了支付，导致支付回调后库存已被释放
        Integer timeLimit = 15; // 目前先延时15分钟，一般支付回调最多都是几分钟内就回调过来了
        long timestamp = LocalDateTime.now().plusMinutes(timeLimit)
                .atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
    ...
    public String productOrderPayCallBack(WxJsPayCallBackResponse response) {
        ...
        // 发送微信订阅消息
        if (payStatus == 1) {
            wechatSubscribeMessageService.notifyOrderPaidToShopManagers(orderInfo);
        }
    ...
    public Result productOrderRefund(ProductOrderRefundRefundParam param, String token) {
        ...
        ProductOrderRefundApply refundApply = null;
        try {
            refundApply = new ProductOrderRefundApply();
            ...
            orderLog.setUserId(userId);
            orderLog.setUserName(orderInfo.getNickName());
            orderLog.setHandle("申请退款");
            orderLog.setDetails(orderInfo.getNickName() + "  " + orderInfo.getPhone() + "申请退款");
            orderLog.setCreateTime(now);
            productOrderLogMapper.insert(orderLog);
        ...
        // 发送订阅消息
        if (refundApply != null) {
            wechatSubscribeMessageService.notifyRefundApplyToShopManagers(orderInfo, refundApply);
        }
