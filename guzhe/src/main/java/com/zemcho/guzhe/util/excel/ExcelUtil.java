...
public class ExcelUtil {
    ...
    /**
     * 带自定义 Handler 的导出方法（用于处理复杂的表头统计等）
     */
    public static <T> void exportToWeb(HttpServletResponse response, List<T> data, String fileName, String sheetName,
                                       Class<T> clazz, List<WriteHandler> handlers) {
        exportToWeb(response, data, fileName, sheetName, clazz, handlers, 6);
    }

    /**
     * 带自定义 Handler 的导出方法（可指定表头起始偏移行）
     */
    public static <T> void exportToWeb(HttpServletResponse response, List<T> data, String fileName, String sheetName,
                                       Class<T> clazz, List<WriteHandler> handlers, int relativeHeadRowIndex) {
        try {
