...
class Login extends React.Component {
  ...
  getData = () => {
    this.setState(
      ...
      async () => {
        //管理员列表
        const res = await post(`/guzhe/admin/lists`, {
        ...
        const ress = await post(`/guzhe/role/lists`, {
          ageSize: 999,
        });
        if (ress && ress.code === 10000) {
          this.setState({
            roleList: ress.data.list,
          });
        } else {
  ...
  deletes = async (ids) => {
    if (ids === undefined) {
      ...
      const res = await post(`/guzhe/admin/delete`, {
        deleteIds: ids,
      });
      if (res && res.code === 10000) {
  ...
  setStatus = async (value, values) => {
    const res = await post(`/guzhe/admin/status/set`, {
      status: value ? 1 : 0,
      changeIds: [values.id],
    });
    if (res && res.code === 10000) {
    ...
    const columns = [
...

      {
        ...
        render: (text, record) => {
          return (
            <div>
              {record.roles && record.roles.map((as) => as.name).join('、')}
            </div>
          );
        },
      },
