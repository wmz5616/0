...
class NoticeNotice extends React.Component {
  ...
  componentDidMount() {
        ...
        ? [
            {
              ...
              name: 'image.png',
              status: 'done',
              url: xx.legalLicenceFrontUrlLocal,
            },
          ]
        : [], //身份证正面
        ...
        ? [
            {
              ...
              name: 'image.png',
              status: 'done',
              url: xx.legalLicenceBackUrlLocal,
            },
          ]
        : [], //身份证反面
        ...
        ? [
            {
              ...
              name: 'image.png',
              status: 'done',
              url: xx.openAccountLicenceUrlLocal,
            },
          ]
        : [], //*开户许可证照片
    });
    ...
    this.formRef.current.setFieldsValue({
      ...
      businessProvince: [
        xx && xx.businessProvince,
        xx && xx.businessCity,
        xx && xx.businessCounty,
      ],
      ...
      businessBeginDate:
        ...
        (xx.businessEndDate == '长期'
          ...
          : xx.businessBeginDate
          ? [
              moment(xx.businessBeginDate, 'YYYY-MM-DD'),
              moment(xx.businessEndDate, 'YYYY-MM-DD'),
            ]
          : undefined),
      ...
      cardBeginDate: xx.cardEndDate
        ? xx.cardEndDate == '长期'
          ...
          : [
              moment(xx.cardBeginDate, 'YYYY-MM-DD'),
              moment(xx.cardEndDate, 'YYYY-MM-DD'),
            ]
        : undefined,
          ...
          this.setState(
            {
                ...
                values.operationType == 1 || true
                  ? {
                      ...
                      businessProvince: values.businessProvince
                        ? values.businessProvince[0]
                        : '', //营业地址省
                      businessCity: values.businessProvince
                        ? values.businessProvince[1]
                        : '', //营业地址市
                      businessCounty: values.businessProvince
                        ? values.businessProvince[2]
                        : '', //营业地址区
              ...
              legalLicenceFrontUrl:
                this.state.IdPhoto &&
                this.state.IdPhoto.length > 0 &&
                this.state.IdPhotoB, //身份证正面
              legalLicenceBackUrl:
                this.state.IdPhotoB &&
                this.state.IdPhotoB.length > 0 &&
                this.state.IdPhotoB, //身份证反面
              openAccountLicenceUrl:
              ...
              this.setState(
                ...
                async () => {
                  ...
                  const params = {
                    ...
                    licenceUrl: this.state.licenceUrls
                      ? this.state.licenceUrls.length > 0 &&
                  ...
                  const res = await post('/guzhe/merchant/update', {
                    ...mergedObj,
                  });
  ...
  submits = async (vv) => {
    this.setState({ comfrimloading: true });
    // const { id } = this.props.location.query;
    const searchParams = new URLSearchParams(history.location?.search || '');
    const id = searchParams.get('id');
    ...
    this.setState({ comfrimloading: false });
    if (res && res.code == 10000) {
      message.success(res.msg);
      history.back();
      // this.setState(
          ...
          title: (
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              {this.state?.xx?.applicationStatus == 'REVIEW_BACK' && (
                <div style={{ color: 'red' }}>
                  驳回原因：{this.state?.xx?.auditOpinion}
                </div>
              )}
            </div>
          ),
          ...
          <div className="steps-action">
            ...
            {current == 1 && (
              <>
                <Row>
                  <Col span={5} offset={8}>
                    <Button
                      loading={this.state.comfrimloading}
                      type="primary"
            ...
            {current == 2 && (
              <>
                <Result
                  status={
                    dataDetails.returnMsg !== '请求成功' ? 'error' : 'success'
                  }
                  title={
                ...
                >
                  <p style={{ marginLeft: 42 }}>
                    商户名称： {this.state && this.state.merchantName}
                  </p>
                  <p>
                    进件申请单编号：{dataDetails && dataDetails.applicationNo}
                    {/* <Link
                      to={`MerchantDetails?stadium_id=${this.state.stadium_id}&oo=1`}
                    >
                      <a>查看详情</a>
                    </Link> */}
                  </p>
                  {/* <p>提交时间：{dataDetails.}</p> */}
                  <p style={{ marginLeft: 42 }}>
                    申请状态：
                    {dataDetails &&
                      dataDetails.applicationStatus == 'REVIEWING' && (
                        <span>
                          {' '}
                          <InfoCircleOutlined
                            style={{ color: '#1890ff' }}
                    ...
                    {dataDetails &&
                      dataDetails.applicationStatus == 'REVIEW_BACK' && (
                        <span>
                          {' '}
                    ...
                    {dataDetails &&
                      dataDetails.applicationStatus == 'AGREEMENT_SIGNING' && (
                        <span>
                          {' '}
                          <InfoCircleOutlined
                            style={{ color: '#1890ff' }}
                    ...
                    {dataDetails &&
                      dataDetails.applicationStatus == 'BUSINESS_OPENING' && (
                        <span>
                          {' '}
                          <InfoCircleOutlined
                            style={{ color: '#1890ff' }}
                    ...
                    {dataDetails &&
                      dataDetails.applicationStatus == 'COMPLETED' && (
                        <span>
