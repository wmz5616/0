...
const TransactionDetails = () => {
  ...
  const searchParamsObj = location.search
  ...
  }, [location.query, location.state, location.search]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        if (!text) return '-';
        return record.orderUserPhone
          ? `${text}(${record.orderUserPhone})`
          : text;
      },
    },
    {
          ...
          <Row gutter={16}>
            ...
            <Col className="gutter-row" span={4}>
              <Form.Item label="下单人" name="orderUser">
                <Input placeholder="请输入姓名或手机号" allowClear />
              </Form.Item>
            </Col>
            <Col className="gutter-row" span={4}>
