...
class UserDetails extends React.Component {
  ...
  getwithdrawalData = async () => {
    const res = await post(`/guzhe/order/withdrawal/lists`, {
  ...
  getReacData = async () => {
    ...
    const res = await post(`/guzhe/order/recharge/lists`, params);
    this.setState({
      spinning: false,
    });
    if (res && res.code == 10000) {
  ...
  getData = async () => {
    const res = await post(`/guzhe/order/exchange/lists`, {
      searchField1: this.state.info.id,
      searchStrField1: this.state.searchStrField1,
      searchStrField2: this.state.searchStrField2,
      searchIntStatus: this.state.searchIntStatus,
  ...
  getInfo = async () => {
    ...
    const res = await post(`/guzhe/user/detail`, {
      searchId: id,
    });
    if (res && res.code == 10000) {
      this.setState(
  ...
  exports = () => {
    ...
    const url = `/guzhe/order/withdrawal/export`;
  ...
  export = () => {
    ...
    let xhr = new XMLHttpRequest();
    let fileName =
      this.state.radioValue == 1 ? `兑换商品列表.xls` : `充值记录列表.xls`; // 文件名称
    const url =
      this.state.radioValue == 1
        ? `/guzhe/order/exchange/export`
        : `/guzhe/order/recharge/lists`;
    xhr.open('POST', url, true);
    xhr.responseType = 'blob';
    xhr.setRequestHeader('Content-Type', 'application/json');
    const data = {
