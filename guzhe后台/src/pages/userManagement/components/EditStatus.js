...
class App extends React.Component {
          ...
          this.formRef.current.setFieldsValue({
            ...
            lockExpiredAt:
              records.lockExpiredAt == null
                ? undefined
                : records.lockExpiredAt &&
                  records.lockExpiredAt == '0000-00-00 00:00:00'
                ? undefined
                : dayjs(records.lockExpiredAt, 'YYYY-MM-DD HH:mm'), //开始时间
  ...
  handleOk = () => {
    const { handleOk, dispatch, ids, getData } = this.props;

    this.formRef.current.validateFields().then(async (values) => {
      const res = await post('/guzhe/user/update', {
        id: +ids,
      lockStatus: values.status == 0 ? 0 : 1, //
        lock:
          this.state.statusa == 0 ? this.state.statusa : this.state.statusaa, //
        phone: values.phone,
        lockExpiredAt:
          this.state.statusaa == 1
            ? values.lockExpiredAt.format('YYYY-MM-DD HH:mm:ss')
            : undefined, //时间
