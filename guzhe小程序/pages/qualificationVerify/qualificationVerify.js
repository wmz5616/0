...
Page({
  ...
  comfirm: function () {
    ...
    const params = {
      businessLicense: this.data.fileList?.[0].url || '',
      otherFile: this.data.otherList?.map(ss => ss.url).join(',') || '',
      phone: this.data.contactPhone,
      email: this.data.contactEmail || undefined,
    ...
    // wx.setStorageSync('qualificationCert', params)
    // wx.showToast({
    //   title:'保存成功',
    //   icon: 'success'
    // })
    ...
    qualificationDetail({
      ...
      if (resd && resd.code == 10000) {
        ...
        this.setData({
          ...
          fileList: data?.businessLicense ? data?.businessLicense?.split(',').map(i => ({
            url: i,
          })) : [],
          otherList: data?.otherFile ? data?.otherFile?.split(',').map(i => ({
            url: i,
          })) : []
        })
      }
  ...
  onLoad(options) {
    ...
    this.setData({
      shopAuditId: +shopAuditId,
      shopId: +id,
      searchType: +searchType,
      qualificationCert: options.qualificationCert
    })
    if (this.data.shopId) {
      this.getshopaudit()
    }
